...
try{
 ...
 await page.getByRole('button',{name:'Converti preventivo',exact:true}).click();await page.getByLabel('Preventivo',{exact:true}).selectOption({index:1});await page.getByLabel('Modello pacchetto').selectOption({index:1});await page.getByLabel('Motivazione',{exact:true}).fill('Abbinamento verificato');await page.getByLabel('Confermo l’abbinamento').check();await page.getByRole('button',{name:'Conferma',exact:true}).click();await page.getByRole('dialog').waitFor({state:'hidden'});
 ...
 await nav('Flussi operativi').click();await page.getByRole('button',{name:'＋ Serie di interventi',exact:true}).click();await page.getByLabel('Pacchetto pagato').selectOption({index:1});await page.getByLabel('Data iniziale').fill('2099-01-05');await page.getByLabel('Ora (Roma)').fill('10:00');await page.getByLabel('Lun',{exact:true}).check();await page.getByLabel('Numero totale appuntamenti').fill('3');await page.getByLabel('Squadra',{exact:true}).selectOption({index:1});await page.getByLabel('Servizio',{exact:true}).fill('Pulizia ricorrente');await page.getByRole('button',{name:'Conferma',exact:true}).click();await page.getByRole('dialog').waitFor({state:'hidden'});
