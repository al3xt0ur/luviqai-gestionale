...
export function Account({session,company,team,onSaved,onLogout}:{session:Session;company:any;team:any[];onSaved:()=>Promise<void>;onLogout:()=>void}) {
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>,action:string){
    ...
    try{await post(action,Object.fromEntries(new FormData(form)),session.csrf,session.user.tenantId);if(action==='password'){onLogout();return;}await onSaved();setMessage('Modifiche salvate.');if(action==='user')form.reset();}catch(e:any){setError(e.message);}finally{setBusy(false);}
  }
