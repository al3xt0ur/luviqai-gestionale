...
import {CalendarView,type CalendarIntervention,type CalendarClient} from './calendar';
import './calendar.css';
...
function CalendarPortal({close}:{close:()=>void}){
 ...
 useEffect(()=>{(async()=>{const me=await fetch('/api/me');if(!me.ok)throw Error('Accedi per visualizzare il calendario.');const session=await me.json();const res=await fetch('/api/state',{headers:{'X-Tenant-Context':session.user.tenantId}});if(!res.ok)throw Error('Impossibile caricare il calendario.');setState(await res.json())})().catch(e=>setError(e.message)).finally(()=>setLoading(false))},[]);
